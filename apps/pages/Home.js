...
export default class Home extends Component {
  ...
  constructor() {
    ...
    //grab the toggle data from the database and set them to local values
  ...
  //update the status of the whatsapp toggle in the DB
  snapChangeFunction() {
    this.setState(state => ({
  ...
  //When save is pressed all the current fields are pushed to the database
  _onSaveFunction = () => {
        var db = firebase.database().ref();
        db.child("MAP").child(this.state.eml.substring(0, this.state.eml.length-4)).child("username").on('value', snapshot => {
            db.child("Users").child(snapshot.val()).child("Profile").child("Facebook").update({
    ...
    //when sign out is pressed the user is signed out
  _onSignOutFunction() {
